 ...
 *
 * Trigger via pg_cron every 30 minutes (see migration). No auth required —
 * route is under /api/public/* and is idempotent (status mapping converges).
 */
  ...
  server: {
    handlers: {
      POST: async () => {
        ...
        // 1) Get the active Ameex provider (single tenant for now)
        ...
        if (!provider) {
          return Response.json(
            { ok: false, error: "No active Ameex provider configured", result },
            { status: 200 },
          );
        }

        // 2) Pull the deliveries that still need tracking
        ...
        if (!pending || pending.length === 0) {
          return Response.json({ ok: true, result, durationMs: Date.now() - startedAt });
        }
        ...
        for (const row of pending) {
          ...
          try {
            ...
            // Ameex tracking response shape (from PHP): { TRACKING: { STATUT, STATUT_S, ... } }
            const t =
              parsed?.TRACKING ||
              parsed?.tracking ||
            ...
            // Compare with current delivery status (avoid useless writes)
            const currentDelStatus = row.status;
            const willChange =
              currentDelStatus !== newStatus &&
              !(newStatus === "shipped" && currentDelStatus !== "pending");
      ...
      // Allow GET too so it can be triggered manually from a browser for debugging.
      GET: async ({ request }) => {
        // Reuse POST handler
        const postHandler = (Route.options as any).server.handlers.POST;
        return postHandler({ request, params: {}, context: {} } as any);
      },
    },
  },
});
